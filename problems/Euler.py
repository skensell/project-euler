#This is taken from blog.dreamshire.com.  It's just a list
#of functions and routines which are often called in
...
class Fibonacci(object):  
    ...
    def __getitem__(self, n):  #so suppose seq = Fibonacci(), then this is seq[n]
        self._fib(n) #Here it calls the function 3 down from here
        return self.fib[n]
 
    def __getslice__(self, start, end):  #seq[start:end]
        self._fib(max(start,end,len(self.fib)))
        return self.fib[start:end]
 
    def __call__(self, n): #seq(n), gives the same as seq[n]
        return self.__getitem__(n)
 
    def _fib(self, n):
        for i in range(len(self.fib), n+1): # seq._fib adds entries up to n
